...
template<Int DIM> struct KDtree {
	...
	void nnearest(Point<DIM> pt, Int *nn, Doub *dn, Int n);
...
template<Int DIM> Int KDtree<DIM>::nearest(Int jpt) {
	...
	while (ntask) {
		k = task[ntask--];
		if (dist(boxes[k],ptss[jpt]) < dnrst) {
			if (boxes[k].dau1) {
				task[++ntask] = boxes[k].dau1;
				task[++ntask] = boxes[k].dau2;
			} else {
...
template<Int DIM> void KDtree<DIM>::nnearest(Point<DIM> pt, Int *nn, Doub *dn, Int n)
{
	Int i,k,ntask,kp;
	Int task[50];
	Doub d;
